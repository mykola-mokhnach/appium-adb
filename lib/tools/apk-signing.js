...
import { getJavaForOs, getJavaHome, rootDir } from '../helpers.js';
...
apkSigningMethods.signWithDefaultCert = async function (apk) {
  ...
  try {
    const apkSigner = await this.getBinaryFromSdkRoot(`apksigner${system.isWindows() ? '.bat' : ''}`);
    const cmd = [apkSigner, 'sign',
      '--key', DEFAULT_PRIVATE_KEY,
      '--cert', DEFAULT_CERTIFICATE,
      apk];
    log.debug(`Starting apksigner: '${cmd}'`);
    await exec(system.isWindows() ? 'cmd.exe' : 'bash', cmd);
  } catch (err) {
...
apkSigningMethods.signWithCustomCert = async function (apk) {
  ...
  try {
    ...
    const jarsigner = path.resolve(javaHome, 'bin', `jarsigner${system.isWindows() ? '.exe' : ''}`);
    try {
...
apkSigningMethods.checkApkCert = async function (apk, pkg) {
  ...
  try {
    const apkSigner = await this.getBinaryFromSdkRoot(`apksigner${system.isWindows() ? '.bat' : ''}`);
    const cmd = [apkSigner, 'verify', apk];
    verificationFunc = async () => await exec(system.isWindows() ? 'cmd.exe' : 'bash', cmd);
  } catch (e) {
